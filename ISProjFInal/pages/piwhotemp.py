...
class VoiceAuthenticator:
    ...
    def authenticate_user(self, username, audio_file):
        try:
            ...
            score, prediction = self.speaker_model.verify_files(audio_file, stored_file)
            ...
            print(passphrase_match_score)
            print(transcribed_text.lower())
            ...
            return ((prediction.item() or score.items() > 50) and
                    passphrase_match_score >= 70)
...
def main():
    ...
    usernameauth = st.text_input("Enter username to authenticate", key="auth_username")

    col1, col2 = st.columns(2)
    with col1:
        if st.button("Generate QR"):
            st.switch_page("pages/qr1.py")
        if st.button("SickDoor"):
            if st.session_state.authcount >= 3:
                ...
                try:
                    ...
                    if audio_file and os.path.exists(audio_file):
                        with st.spinner("Authenticating..."):
                            authenticated = authenticator.authenticate_user(usernameauth, audio_file)
